...
function szz_flux((x, y, model, loss), kappa, λref, Niter, h, thin)
    xx = [loss(model, x, y),]
    nobs = size(x, 2)
    ...
    while(i < Niter)
        ...
        τ0, k0, ev  = eventszz(θ, v, vc, s, dt, ∇Uθ.*v, λref, κs)
        t += τ0
        θ .+= v*τ0
...
model1, trace =szz_flux((x, y, model, loss),  1.0, 0.0, 100_000, 0.00005, 1000)
